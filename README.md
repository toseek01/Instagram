...
𝑀(j, 𝑆) - Маржинальный вклад. UserID которые посетили канал Xj + S (в том числе и нулевое множество) и совершили некий вклад</br>
![Image alt](https://github.com/toseek01/Instagram/blob/master/illustration/marg.png)</br>
Shapley value - Формула расчета аттрибуции для всех каналов (Shapley attribution formula)</br>
...
Ф𝑗 - Аттрибутированный вес канала (Shapley value)</br>
total campaign - Сумма конверсий за всю кампанию
Shapley values - Аттрибутированные веса каналов 
...
не делает никакого разделения был ли канал на первом месте или последнем,полностью игнорирует последовательность каналов и их места в цепи. Позиция канала в цепи никаким образом не повлияет на итоговый аттрибутированный вес этого канала. Задача Ordered Shapley состоит в том,чтобы устранить этот недостаток и попытаться ответить на вопрос, как же распределялся вес каждой позиции в цепочке внутри канала</br>
