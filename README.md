...
Надо заметить ,что Order Shapley так же как и Shapley обладает некоторыми свойсвтвами благодаря которым мы сможем проверить свебя,правильно ли мы все рассчитали.
 
 - Order Shapley value для каналов.Сумма всех весов позиций внутри канала должна равняться весу самого канала.В виде уравнения мы это можем записать так
 ![Image alt](https://github.com/toseek01/Instagram/blob/master/illustration/touchpoint.png)</br>
 Это свойство автоматически проверятся в методе ***Calc*** и логируется, если неравество не выполняется, то канал/каналы на которых произошла ошибка будут записаны во внутренню переменную ***errors_order***</br>
 
 - Order Shapley value для позиций. Это свойство автоматически не проверяется и благодаря ему нет возможности првоеприть правильно ли были сделаны вычисления,но мы считаем нужным его на всякий случай указать, потому как оно в будущем может оказаться для кого-то или чего-то полезным.</br>
  ![Image alt](https://github.com/toseek01/Instagram/blob/master/illustration/orderprop.png)</br>
  Суммарный аттрибутированный вес i-ой позиции равен сумме всех i-ых весов всех каналов
