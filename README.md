...
***CreditChannel*** </br>
Оставляет только каналы с shapley value > 0 ,чтобы Ordered Shapley не тратил время на расчетов весов позиций для каналов чей вес равен 0</br>
***VectorizationOrder***</br> Конвертирует цепочку в вектор ,содержищий их ID .Пример:</br>
...
***MatrixOrder***</br>
По сути ,этот метод выполняет аналогичные операции что и модуль **Vectorization**  в классе `shapleyLib`, а именно формирует матрицу расчетов для Shapley, с той лишь разницей что в этой матрице будут учитываться знания и кол-ве наличия данного канала в конкретной цепи</br>
***Cardinality*** </br> Расчет мощности множества (см теорию "How to calculate cardinality of set")
***TouchpointR***</br> Рассчитывает веса каждой позиции внутри одного канала
***Calc***</br> Запускает Order Shapley
